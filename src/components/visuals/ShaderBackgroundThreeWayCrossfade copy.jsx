...
// --- GLSL Shader Source for 3-way crossfade ---
...
// Core shader computation for parameter vec4
 ...
 *   modC: vec4 array for shader C
 *   from: int (0=A, 1=B, 2=C)
 *   to: int (0=A, 1=B, 2=C)
 *   fade: float 0-1, controls crossfade
 */
export default function ShaderBackgroundThreeWayCrossfade({
  modA = [0, 1, 2, 0],
  modB = [4, 1, 8, 2],
  modC = [6, 2, 8, 4],
  from = 0,
  ...
  useEffect(() => { modCRef.current = modC; }, [modC]);
  ...
  useEffect(() => {
    ...
    const modCLocation = gl.getUniformLocation(program, "u_modC");
    ...
    const render = (currentTime) => {
      ...
      gl.uniform1i(fromLocation, fromRef.current);
      gl.uniform1i(toLocation, toRef.current);
      gl.uniform4fv(modALocation, modARef.current);
      gl.uniform4fv(modBLocation, modBRef.current);
      gl.uniform4fv(modCLocation, modCRef.current);
